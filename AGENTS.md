...
- Keep the navigation above all page content, including the compact scrolling state. The header uses only the white Guit v.8 symbol. Blue `#0125ff` is the primary interface accent; retain original artwork colors. Rename the home section to Social Media. FiveM gallery order is Social media, Logotipos, Logotipos animados, then Ícones Loja VIP; use supplied FiveZ, Riverside, and Exodus artwork for social and VIP assets.
